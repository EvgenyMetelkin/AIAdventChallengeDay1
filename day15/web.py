...
from swarm import SwarmOrchestrator, SwarmStage, STAGE_LABELS, STAGE_DESCRIPTIONS, INVARIANT_CHECKER_SYSTEM_PROMPT
...
# ============================================================
# Инварианты — чтение и редактирование
# ============================================================

@app.get("/api/invariants")
async def get_invariants(state: AppState = Depends(get_state), user_id: Optional[str] = None):
    """Получить инварианты текущего пользователя."""
    user = await _resolve_user(state, user_id)
    user_dir = os.path.join(USERS_DIR, user.user_id)
    invariants_file = os.path.join(user_dir, "invariants.md")
    ...
    if os.path.exists(invariants_file):
        try:
            ...
            raise HTTPException(status_code=500, detail=f"Error reading invariants: {str(e)}")
    else:
        content = "# Invariants\n\n(no invariants defined — add bullet points below)\n"

    invariants_list = parse_invariants_md(content)

    return {
        "user_id": user.user_id,
        "content": content,
        "invariants": invariants_list,
        "count": len(invariants_list),
    }


class InvariantsUpdateRequest(BaseModel):
    content: str


@app.put("/api/invariants")
async def update_invariants(
    req: InvariantsUpdateRequest,
    state: AppState = Depends(get_state),
    user_id: Optional[str] = None,
):
    """Обновить инварианты пользователя (атомарная запись)."""
    user = await _resolve_user(state, user_id)
    user_dir = os.path.join(USERS_DIR, user.user_id)
    invariants_file = os.path.join(user_dir, "invariants.md")
    ...
    try:
        # Atomic write
        from user import _atomic_write_text
        _atomic_write_text(invariants_file, req.content)
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Error writing invariants: {str(e)}")

    invariants_list = parse_invariants_md(req.content)
    logger.info(f"Updated invariants for user {user.name} ({user.user_id}): {len(invariants_list)} invariants")
...
class SwarmActionRequest(BaseModel):
    ...
    text: Optional[str] = None  # User input text for user_input/refine_plan actions
...
async def swarm_create(req: SwarmCreateRequest, state: AppState = Depends(get_state)):
    ...
    # Load user invariants
    invariants = []
    invariants_file = os.path.join(USERS_DIR, user.user_id, "invariants.md")
    if os.path.exists(invariants_file):
        try:
            with open(invariants_file, "r", encoding="utf-8") as f:
                content = f.read()
            invariants = parse_invariants_md(content)
        except Exception as e:
            logger.warning(f"Failed to load invariants for user {user.user_id}: {e}")

    try:
        task = await swarm.create_task(req.description, user.user_id, invariants=invariants)
        return {
            "status": "ok",
            "task": task.to_dict(),
        }
...
async def swarm_action(task_id: str, req: SwarmActionRequest, state: AppState = Depends(get_state)):
    ...
    action_map = {
        ...
        "restart_stage": swarm.restart_stage,
        "cancel": swarm.cancel,
        "user_input": swarm.handle_user_input,
        "refine_plan": swarm.refine_plan,
    }
    ...
    try:
        if req.action in ("user_input", "refine_plan"):
            if not req.text:
                raise HTTPException(status_code=400, detail=f"Action '{req.action}' requires 'text' field")
            task = await handler(task_id, req.text)
        else:
