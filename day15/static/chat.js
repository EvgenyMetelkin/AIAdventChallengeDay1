...
async function sendMessage() {
    ...
    // Route to swarm if there's an active task (not DONE/CANCELLED/FAILED)
    if (swarmTaskId && swarmTaskData) {
        const stage = swarmTaskData.current_stage;
        const activeStages = ['idle', 'planning', 'plan_review', 'executing', 'exec_review',
                              'validating', 'validation_review', 'finishing', 'paused', 'failed'];
        if (activeStages.includes(stage)) {
            await swarmInput(message);
            return;
        }
    }
...
function renderSwarmView(data) {
    ...
    // Questions display (Day 15)
    updateSwarmQuestions(task);

    // Plan display (Day 15)
    updateSwarmPlan(task);
...
// ---- Day 15: Questions and Plan display ----

function updateSwarmQuestions(task) {
    const panel = document.getElementById('swarmQuestions');
    const progress = document.getElementById('swarmQuestionsProgress');
    const list = document.getElementById('swarmQuestionsList');
    const hint = document.getElementById('swarmQuestionsHint');

    if (!panel || !list) return;

    const questions = task.pending_questions || [];
    const answers = task.answers || [];
    const qIndex = task.question_index || 0;
    const waiting = task.waiting_for_answers;
    const stage = task.current_stage;

    if (!waiting || stage !== 'planning' || questions.length === 0) {
        panel.style.display = 'none';
        return;
    }

    panel.style.display = 'block';
    if (progress) progress.textContent = `${qIndex}/${questions.length} отвечено`;

    let html = '';
    for (let i = 0; i < questions.length; i++) {
        let cls = 'pending';
        if (i < qIndex) cls = 'answered';
        else if (i === qIndex) cls = 'current';

        const answer = answers[i] || '';
        html += `<div class="swarm-question-item ${cls}">`;
        html += `<span class="swarm-question-num">${i + 1}.</span> ${escapeHtml(questions[i])}`;
        if (answer) {
            html += `<div class="swarm-question-answer">→ ${escapeHtml(answer)}</div>`;
        }
        html += `</div>`;
    }
    list.innerHTML = html;
...
function updateSwarmPlan(task) {
    const panel = document.getElementById('swarmPlanArea');
    const content = document.getElementById('swarmPlanContent');

    if (!panel || !content) return;

    const planText = task.plan_text || '';
    const stage = task.current_stage;

    // Show plan when in PLAN_REVIEW or when plan_text is available
    if ((stage === 'plan_review' || stage === 'planning') && planText) {
        panel.style.display = 'block';
        content.innerHTML = formatMarkdown(planText);
    } else {
        panel.style.display = 'none';
    }
}
...
async function refreshSwarmTaskUI() {
    if (!swarmTaskId) return;
    try {
        const response = await fetch(`/api/swarm/tasks/${swarmTaskId}`);
        if (!response.ok) throw new Error(`HTTP ${response.status}`);
...
function updateSwarmControls(stage) {
    ...
    const allBtns = [
        ...
        'swarmBtnStartValidation', 'swarmBtnFinish', 'swarmBtnRefine', 'swarmBtnRetry',
    ...
    switch (stage) {
        case 'idle':
            ...
            break;
        case 'plan_review':
            show('swarmBtnConfirm');  // approve → start_execution
            show('swarmBtnRefine');   // refine plan (Day 15)
...
async function swarmAction(action) {
    ...
    try {
        ...
        const actionLabels = {
            ...
            user_input: 'Отправлено',
            refine_plan: 'План уточнён'
        };
...
// ---- Day 15: Input routing ----

async function swarmInput(message) {
    if (!swarmTaskId) return;
    if (isWaiting) { showToast('Подождите...', 'info'); return; }
    ...
    try {
        const response = await fetch(`/api/swarm/tasks/${swarmTaskId}/action`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ action: 'user_input', text: message })
        });
        if (!response.ok) {
            const errData = await response.json().catch(() => ({}));
        ...
        // If stage changed to plan_review, show success
        if (swarmTaskData.current_stage === 'plan_review' && swarmTaskData.plan_text) {
...
async function swarmRefinePlan() {
    if (!swarmTaskId) return;
    if (isWaiting) { showToast('Подождите...', 'info'); return; }

    const refineText = prompt('Введите уточнения к плану (что нужно изменить, добавить или уточнить):');
    if (!refineText || !refineText.trim()) return;
    ...
    try {
        const response = await fetch(`/api/swarm/tasks/${swarmTaskId}/action`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ action: 'refine_plan', text: refineText.trim() })
        });
        if (!response.ok) {
            const errData = await response.json().catch(() => ({}));
            throw new Error(errData.detail || `Ошибка (${response.status})`);
        ...
        await refreshSwarmTaskUI();
        await loadSwarmTasks();
        showToast('План обновлён с учётом уточнений', 'success');
    } catch (err) {
        showToast(`Ошибка: ${err.message}`, 'error');
    } finally {
        isWaiting = false;
