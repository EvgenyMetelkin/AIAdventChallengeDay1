    ...
    async def start_planning(self, task_id: str) -> SwarmTask:
        ...
        try:
            ...
            if not check_result["passed"]:
                # Violations found — mark as failed
                ...
                self._save_task(task)
                logger.warning(f"Planning failed invariant check for task {task_id}: {len(check_result['violations'])} violations")
                return task
...
Rate completeness, identify gaps, and recommend whether to proceed."""
            ...
            if not check_result["passed"]:
                ...
                self._save_task(task)
                logger.warning(f"Validation failed invariant check for task {task_id}: {len(check_result['violations'])} violations")
                return task
...
Create a comprehensive final report that summarizes the entire process."""
    ...
    async def retry_stage(self, task_id: str) -> SwarmTask:
        ...
        if task.current_stage in review_stages:
            ...
            # Use restart_stage logic for failed invariant check
    ...
    async def restart_stage(self, task_id: str) -> SwarmTask:
        """Restart the current failed stage after invariant violations.

        Clears the current stage's result and returns to the pre-stage
        so the stage can be re-run. Only allowed from FAILED state.
        """
        task = self._get_task(task_id)
        if task.current_stage != SwarmStage.FAILED:
            raise ValueError(
                f"Can only restart from FAILED state, current: {task.current_stage.value}"
            )

        # Determine which stage failed and where to go back to
    ...
    async def _run_agent(self, system_prompt: str, user_message: str, invariants: Optional[List[str]] = None) -> str:
        ...
        Uses the httpx client directly since we need a custom system prompt.
        Optionally injects invariants into the prompt.

        Args:
            system_prompt: The agent's system prompt
            user_message: The user message for the agent
            invariants: Optional list of invariant strings to inject
        """
        # Build the full system prompt with invariants if provided
    ...
    async def _check_invariants(self, invariants: List[str], artifact_text: str, stage_name: str) -> dict:
        ...
        Uses a separate LLM call with the invariant checker system prompt.

        Args:
            invariants: List of invariant strings
            artifact_text: The text of the artifact to check
            stage_name: Name of the stage being checked (for logging)

        Returns:
            dict with keys: "passed" (bool), "violations" (list of violation dicts),
            "raw_response" (str)
        """
        if not invariants:
        ...
        try:
            ...
            # If we can't parse, treat as a soft failure — return ambiguous result
            # but don't block the pipeline entirely
